...
export function Prompt(props: PromptProps) {
  ...
  function clearPrompt() {
    const trimmedInput = store.prompt.input.trim()
    const shouldSave =
      kv.get("clear_prompt_save_history", false)
        ? trimmedInput.length > 0 || store.prompt.parts.length > 0
        : trimmedInput.length >= DRAFT_RETENTION_MIN_CHARS || store.prompt.parts.length > 0
    if (shouldSave) {
      history.append({
        ...store.prompt,
        mode: store.mode,
